...
class Listar extends Component
{
    ...
    {
        return view('index.mixagem.listar');
    }
    ...
    public function cancelarAgendamento($idMixagem){
        ...
        $this->emit('atrazar_redirect', ['caminho' => '/mixagem/listar', 'tempo' => 2500]);
    }
    ...
    public function msgParaRegistroActividade($cliente_id, $grupo_id)
    {
        ...
        if (!empty($nomeCliente) && !empty($mebroEmGrupo)) {
            $this->registrarActividade("<b><i class='bi bi-check-circle-fill text-success'></i> Cancelou um agendamento de mixagem para cliente $nomeCliente->name do grupo " . $this->buscarGrupo($mebroEmGrupo->grupo_id)->nome . " </b> <hr>" . $this->infoDispositivo, "normal", Auth::user()->id);
        } elseif ($nomeCliente) {
            $this->registrarActividade("<b><i class='bi bi-check-circle-fill text-success'></i> Cancelou um agendamento de mixagem para cliente $nomeCliente->name </b> <hr>" . $this->infoDispositivo, "normal", Auth::user()->id);
        } elseif ($nomeGrupo) {
            $this->registrarActividade("<b><i class='bi bi-check-circle-fill text-success'></i> Cancelou um agendamento de mixagem para o grupo $nomeGrupo->nome </b> <hr>" . $this->infoDispositivo, "normal", Auth::user()->id);
        }
    }
