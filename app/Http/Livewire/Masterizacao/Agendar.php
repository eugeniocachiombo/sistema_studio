...
class Agendar extends Component
{
    ...
    public function dataMinimaAgendamento()
    {
        ...
        if (!empty($maiorData)) {
            ...
            if($this->dataMasterizacao == null){
                $this->dataMasterizacao =  $this->dataMin;
            }
        } else {
    ...
    public function msgParaRegistroActividade($cliente_id, $grupo_id)
    {
        ...
        if (!empty($nomeCliente) && !empty($mebroEmGrupo)) {
            $this->registrarActividade("<b><i class='bi bi-check-circle-fill text-success'></i> Fez um agendamento de masterização para cliente $nomeCliente->name do grupo " . $this->buscarGrupo($mebroEmGrupo->grupo_id)->nome . " </b> <hr>" . $this->infoDispositivo, "normal", Auth::user()->id);
        }else if ($nomeCliente) {
